...
class CnnLstmSig(nn.Module):
    def __init__(self, ninput=63, channels=256, nhid=100, nlayers=1, dropout=0.5, num_classes=3, bidirectional=False):
        super(CnnLstmSig, self).__init__()
        ...
        self.num_classes = num_classes
        self.linear_hid2 = nn.Linear(nhid, num_classes)
        self.linear_hid_activ2 = nn.Sigmoid()
...
class CnnRaw(nn.Module):
    def __init__(self, ninput=63, channels=256, nhid=100, dropout=0.5, num_classes=3):
        ...
        self.enc = nn.Sequential(
            nn.Conv1d(1, channels, kernel_size=256, stride=4, padding=128, bias=False),
            nn.BatchNorm1d(channels),
            nn.LeakyReLU(),
            nn.Dropout(p=dropout),
            nn.Conv1d(channels, channels, kernel_size=48, stride=1, padding=23, bias=False),
            nn.BatchNorm1d(channels),
            nn.LeakyReLU(),
            nn.Dropout(p=dropout),
            ...
            nn.LeakyReLU(),
            nn.Dropout(p=dropout),
            nn.Conv1d(channels, channels, kernel_size=8, stride=1, padding=4, bias=False),
            nn.BatchNorm1d(channels),
            nn.LeakyReLU(),
            nn.Dropout(p=dropout),
            nn.Conv1d(channels, channels, kernel_size=8, stride=1, padding=3, bias=False),
            ...
            nn.LeakyReLU(),
            nn.Dropout(p=dropout),
            nn.Conv1d(channels, ninput, kernel_size=32, stride=1, padding=15, bias=False),
            nn.BatchNorm1d(ninput),
            nn.LeakyReLU(),
            LambdaLayer(lambda x: x.transpose(1,2)),
        )
...
def load_model(path):
    ...
    if ntype == 'cnn':
        ...
        model = CnnRaw(ninput=params["input_size"], channels=params["channels"], nhid=params["hidden_size"], dropout=0, num_classes=params["class_num"])
    elif ntype == 'cnn_sim':
        ...
        if sigmoid:
            model = CnnLstmSig(ninput=params["input_size"], channels=params["channels"], nhid=params["hidden_size"], nlayers=params["num_layers"], dropout=0, bidirectional=params["biLSTM"], num_classes=params["class_num"])
        else:
